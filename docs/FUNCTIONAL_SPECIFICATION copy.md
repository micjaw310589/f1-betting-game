...
* **Typ zakładu (Bet Type)** – Zdefiniowana kategoria przewidywania wyniku sportowego (np. zwycięzca wyścigu, TOP 3 kierowców, najszybsze okrążenie, liczba kierowców, którzy nie ukończyli wyścigu – DNF, najszybszy pit stop).
...
<!-- * **Tabela liderów (Leaderboard)** – Globalny oraz okresowy ranking użytkowników uporządkowany malejąco według sumy posiadanych wirtualnych punktów. -->
...
* **Częściowa wygrana (Partial Win)** – Sytuacja, w której użytkownik poprawnie wytypował część elementów zakładu wielopozycyjnego (np. 2 z 3 kierowców na podium), co skutkuje proporcjonalnym przyznaniem części nagrody punktowej.
...
* Implementacja 13 kluczowych modułów frontendowych i odpowiadających im punktów końcowych API opisanych w niniejszej specyfikacji.
...
* Bezpieczny system uwierzytelniania oparty o ASP.NET Core Identity oraz tokeny JWT wraz z automatycznym mechanizmem ich odświeżania w tle.
...
|-----------|-------------|-------|-------------------|--------------------------------------------|
| **R-01** | Niedostępność lub nagła zmiana struktury danych w OpenF1 API. | Krytyczny | Średnie | Wdrożenie warstwy pamięci podręcznej (Cache Redis/DB) zawierającej ostatnie poprawne dane. Automatyczne alerty e-mail do administratorów o awarii integracji. Udostępnienie panelu manualnego nadpisywania wyników (*Override*). |
...
| **R-05** | Podatności bezpieczeństwa (SQL Injection, XSS, nadużycia API). | Krytyczny | Średnie | Wykorzystanie ORM Entity Framework Core (automatyczna parametryzacja zapytań SQL), sanityzacja danych wejściowych w Angularze przed renderowaniem HTML, wdrożenie globalnego mechanizmu Rate Limitingu na serwerze API. |
...
* **Czas ładowania strony (Page Load Time)** – Czas renderowania i pełnej gotowości interfejsu dla kluczowych widoków (Dashboard, Lista Wyścigów) nie może przekroczyć 2 sekund przy przepustowości łączą sieciowego na poziomie 3G.
...
* **Współbieżność (Concurrency)** – System w konfiguracji bazowej musi bezawaryjnie obsługiwać minimum 1000 zalogowanych użytkowników jednocześnie wykonujących operacje w bazie danych podczas weekendu wyścigowego.

### 10.2 Bezpieczeństwo (Security)
* **Ochrona haseł** – Hasła użytkowników podlegają bezwzględnemu haszowaniu przed zapisem w bazie danych przy użyciu silnego algorytmu kryptograficznego (np. BCrypt lub PBKDF2). Wprowadzanie otwartego tekstu do bazy jest zabronione.
...
* **Autoryzacja** – Przechowywanie tokenów autoryzacyjnych JWT musi odbywać się w sposób uniemożliwiający ataki typu XSS i CSRF (rekomendowane użycie ciasteczek z flagami `HttpOnly`, `Secure` oraz `SameSite=Strict`).
...
* **Retencja danych i Backup** – Automatyczna kopia zapasowa bazy danych MS SQL Server musi być wykonywana codziennie w godzinach nocnych i przechowywana w bezpiecznej, odizolowanej lokalizacji przez okres minimum 30 dni.

### 10.4 Skalowalność (Scalability)
* **Skalowanie horyzontalne** – Backend API zbudowany w ASP.NET Core musi mieć charakter bezstanowy (Stateless), co umożliwi jego natychmiastowe skalowanie horyzontalne poprzez dodawanie kolejnych instancji kontenerów za balanserem obciążenia (Load Balancer).
* **Strategia Cachingowa** – Często odpytywane i rzadko zmieniające się dane (np. kalendarz wyścigów, profile kierowców) muszą być cache'owane przy użyciu pamięci podręcznej Redis, odciążając relacyjną bazę danych.

### 10.5 Użyteczność (Usability) i Dostępność
...
* **Standardy Dostępności** – Kod HTML i style CSS muszą spełniać wytyczne standardu dostępności cyfrowej WCAG 2.1 na poziomie AA, zapewniając odpowiedni kontrast tekstu oraz pełną nawigację z poziomu klawiatury dla osób niedowidzących.

### 10.6 Konserwowalność (Maintainability)
...
* **Testy Automatyczne** – Pokrycie kodu testami jednostkowymi i integracyjnymi (xUnit dla C#, Jasmine/Karma dla Angulara) dla kluczowej logiki biznesowej (obliczanie punktów, walidacja zakładów, interceptor) musi wynosić minimum 80%.

---

## 11. Wdrożenie systemu
Proces wdrożenia systemu i infrastruktura docelowa obejmują następujące obszary technologiczne:

### 11.1 Środowisko uruchomieniowe i Architektura
...
* Kontener Bazy Danych – Instancja serwera bazodanowego Microsoft SQL Server.
* Kontener Pamięci Podręcznej – Serwer Redis optymalizujący zapytania odczytu.

### 11.2 Potok CI/CD (Continuous Integration / Continuous Deployment)
Automatyzacja procesów budowania i wdrażania realizowana jest przy użyciu narzędzia **GitHub Actions** lub **Azure DevOps**:
1. **Faza CI (Integracja)** – Każdy wypchnięty kod (Push) lub żądanie zmiany (Pull Request) do gałęzi `main` uruchamia automatyczny potok, który pobiera zależności, przeprowadza statyczną analizę kodu (Linter), buduje aplikację frontendową i backendową oraz wykonuje komplet testów automatycznych (jednostkowych i integracyjnych). Wykrycie jakiegokolwiek błędu przerywa potok.
2. **Faza CD (Wdrożenie)** – Po pomyślnym zakończeniu fazy CI system automatycznie buduje produkcyjne obrazy Docker, nadaje im tagi z numerem wersji i wypycha do rejestru obrazów (np. Docker Hub, Azure Container Registry). Następnie potok wywołuje skrypt aktualizacyjny na serwerze docelowym, który pobiera nowe obrazy i restartuje usługi bez przestoju systemu (Rolling Update / Blue-Green Deployment).

### 11.3 Monitoring, Logowanie i Utrzymanie
* **Logowanie zdarzeń** – System wykorzystuje bibliotekę **Serilog** do strukturalnego logowania wszystkich zdarzeń systemowych, błędów integracji z OpenF1 oraz akcji moderacyjnych administratorów. Logi są agregowane w centralnym systemie zarządzania logami (np. ELK Stack lub Seq).
* **Monitorowanie wydajności** – Nadzór nad zużyciem zasobów serwera (CPU, RAM), czasem odpowiedzi punktów końcowych oraz zdrowiem background workerów realizowany jest za pomocą narzędzi **Prometheus** i **Grafana** lub **Application Insights**, dostarczając administratorom dashboardy operacyjne w czasie rzeczywistym.
